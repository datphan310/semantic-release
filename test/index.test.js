...
test('Make a new release when a commit is forward-ported to an upper branch', async t => {
  ...
  commits.push(...(await gitCommits(['fix: fix on LTS version 1.0.x'], {cwd})));
...
test('Throws "EINVALIDNEXTVERSION" if next release is out of range of the current branch', async t => {
  const {cwd, repositoryUrl} = await gitRepo(true);
  const commits = await gitCommits(['feat: initial release'], {cwd});
  await gitTagVersion('v1.0.0', undefined, {cwd});
  ...
  commits.push(...(await gitCommits(['feat: feat on LTS version 1.x'], {cwd})));
  await gitPush('origin', 'master', {cwd});
...
test('Throws "EINVALIDLTSMERGE" if merge an out of range release in a lts branch', async t => {
  const {cwd, repositoryUrl} = await gitRepo(true);
